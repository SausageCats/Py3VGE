...
def vge_task(arg1, arg2, arg3, arg4="", arg5=None):
    ...
    if arg1 == "finished":
        ...
        joblist = "finished"
        flag_newjob = False
        joblist_shipped = pickle.dumps(joblist, protocol=lowest_protocol)
    #
    ...
    while Flag:
        try:
            ...
            if isinstance(joblist, dict) and flag_newjob:
                ...
                joblist_shipped = pickle.dumps(temp, protocol=lowest_protocol)
            ...
            if isinstance(mesg, bytes):
                ...
                #
                if not flag_newjob:
                    if isinstance(messeage, dict) and joblist == "pipeline_ppid_list":
                        result = messeage
                    elif isinstance(messeage, str) and joblist == "restofjob":
                        result = messeage
    ...
    if flag_newjob:
        ...
        while Flag:
            try:
                ...
                joblist_shipped = pickle.dumps(jobstatus, protocol=lowest_protocol)
                #
                ...
                # mesg = pickle.loads(mesg) if mesg else ''
                time_waitrecv = time.time() - time_waitrecv
                if mesg is not None or mesg.strip() != b"":
                    try:
                        messeage = pickle.loads(mesg)
                    except EOFError:
                        pass
