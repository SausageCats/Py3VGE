...
from VGE.make_vge_jobstatuslist import make_vge_jobstatuslist
                ...
                for sock in inputready:
                    ...
                    if sock is server_sock:
                        ...
                        while True:
                            ...
                            #     break
                            # elif temp[-1:] == b".":
                            #     if mesg[-3:] == b"q\x00." or mesg[-2:] == b"u.":
                            #         break
                        ...
                        if isinstance(messeage, str):
                            if "finished" in messeage:  # the VGE controller will stop.
                                ...
                                flag_byebye = True
                                send_time = time.time()
                                sock.sendall(pickle.dumps(mesg, protocol=lowest_protocol))
                                ...
                                send_time = time.time()
                                sock.sendall(pickle.dumps(temp, protocol=lowest_protocol))
                                send_time = time.time() - send_time
                                del temp
                                sum_send_time += send_time
                                time.sleep(nsleep_aftersend)
                                ...
                                mesg = ""
                                send_time = time.time()
                                sock.sendall(pickle.dumps(task_check["restofjob"], protocol=lowest_protocol))
                                send_time = time.time() - send_time
                                sum_send_time += send_time
                                time.sleep(nsleep_aftersend)
                            ...
                            #
                            if "byebye_from_pipeline" in messeage:
                                temp = int(messeage["pipeline_parent_pid"])
                                if temp in pipeline_parent_pid_list:
                            ...
                            # mark a new pipeline
                            #
                            elif "hello_from_pipeline" in messeage:
                                temp = int(messeage["pipeline_parent_pid"])
                                if temp in pipeline_parent_pid_list:
                            ...
                            #
                                ...
                                if check_max_task == 0:
                                    check_count = -1
                                    if check_jobid in total_joblist:
                                        flag = True
                                        while flag:
                                            try:
                                            ...
                                            if check_job["status"] == "done" or check_job["status"] == "aborted":
                                                if check_job["unique_jobid"] == check_unique_jobid:
                                                    ...
                                                    for okey, ojob in total_joblist.items():
                                                        print("total_list: jobid( % 04i) unique_jobid(%04i) bulk_id(%04i)" % (okey, ojob["unique_jobid"], ojob["bulkjob_id"]))
                                                    time.sleep(30)
                                            else:
                                ...
                                send_time = time.time()
                                sock.sendall(pickle.dumps(check_status, protocol=lowest_protocol))
                                send_time = time.time() - send_time
                                sum_send_time += send_time
                                flag_dispatch = True
                                time.sleep(nsleep_aftersend)
                            ...
                            #
                                ...
                                if messeage is not None or messeage != "":
                                    ...
                                    #
                                    temp = int(messeage["pipeline_parent_pid"])
                                    if temp not in pipeline_parent_pid_list:
                                        #
                                        # update Pipeline parent list
                                        #
                                    ...
                                    #
                                    send_time = time.time()
                                    sock.sendall(pickle.dumps(answer, protocol=lowest_protocol))
                                    send_time = time.time() - send_time
                                    flag_dispatch = True
                                    sum_send_time += send_time
                                    time.sleep(nsleep_aftersend)
                                    ...
                                    #
                                    send_time = time.time()
                                    sock.sendall(pickle.dumps(unique_jobid, protocol=lowest_protocol))
                                    send_time = time.time() - send_time
                                    sum_send_time += send_time
                                    time.sleep(nsleep_aftersend)
